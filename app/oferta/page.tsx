...
const HOTMART_URL = process.env.NEXT_PUBLIC_HOTMART_URL || "/analizador?src=oferta";

const BENEFITS = [
  { icon: "📈", t: "Macro + Micro a la vez", d: "Subes tu chart en temporalidad alta y baja. Dos análisis en paralelo: Código Suizo + SMC/ICT." },
  { icon: "🎯", t: "3 metodologías con %", d: "Cada análisis te da un score en SMC, ICT y Scalping, más veredicto y plan de entrada." },
  { icon: "📊", t: "Con análisis fundamental", d: "Pegas el contexto macro (tasas, DXY, noticias) y la IA lo cruza con tu sesgo técnico." },
];
...
export default function Oferta() {
  return (
    <main className="mx-auto w-full max-w-3xl px-4 py-12 sm:py-20">
      <header className="text-center">
        ...
        <div className="mx-auto mt-8 max-w-md rounded-2xl border border-sage-bright/30 bg-gradient-to-b from-sage-bright/10 to-transparent p-5">
          ...
          <div className="mt-5 flex flex-col gap-2 sm:flex-row">
            <Link href="/analizador?src=oferta" className="rounded-xl bg-sage-bright px-6 py-3 text-base font-semibold text-ink-deep transition hover:bg-sage-light">
              Probar gratis →
            </Link>
      ...
      <section className="mt-16">
        <h2 className="text-center text-sm font-semibold uppercase tracking-widest text-sage-muted">Cómo funciona</h2>
        <div className="mt-4 grid gap-3 sm:grid-cols-3">
          {STEPS.map((s) => (
            <div key={s.n} className="rounded-xl border border-line bg-surface/50 p-4">
              <div className="flex h-8 w-8 items-center justify-center rounded-full bg-sage-bright/15 font-mono text-sm font-bold text-sage-bright">{s.n}</div>
      ...
      <section className="mt-16 text-center">
        ...
        <Link href="/analizador?src=oferta" className="mt-4 inline-block rounded-xl bg-sage-bright px-6 py-3 text-base font-semibold text-ink-deep transition hover:bg-sage-light">
          Probar Swiss Charts →
        </Link>
      </section>
